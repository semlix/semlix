...
thread/process, it will raise ``semlix.store.LockError``.
...
semlix includes a couple of example implementations
(:class:`semlix.writing.AsyncWriter` and :class:`semlix.writing.BufferedWriter`)
...
:meth:`semlix.searching.Searcher.up_to_date`. If the searcher is not up to date,
you can get an up-to-date copy of the searcher using
:meth:`semlix.searching.Searcher.refresh`::
