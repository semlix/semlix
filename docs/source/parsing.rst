...
into *query objects* (objects from the :mod:`semlix.query` module).
...
semlix includes a powerful, modular parser for user queries in the
:mod:`semlix.qparser` module. The default parser implements a query language
similar to the one that ships with Lucene. However, by changing plugins or using
functions such as :func:`semlix.qparser.MultifieldParser`,
:func:`semlix.qparser.SimpleParser` or :func:`semlix.qparser.DisMaxParser`, you
can change how the parser works, get a simpler parser or change the query
language syntax.

(In previous versions of semlix, the query parser was based on ``pyparsing``.
...
.. note::
    ...
    the objects in the :mod:`semlix.query` module. If you are not processing
...
To create a :class:`semlix.qparser.QueryParser` object, pass it the name of the
...
In that case, you can use a :class:`semlix.qparser.MultifieldParser`. This is
...
:meth:`~semlix.qparser.QueryParser.remove_plugin_class` method.
...
The :class:`semlix.qparser.OperatorsPlugin` implements the ability to use AND,
...
The :class:`semlix.qparser.GtLtPlugin` lets you specify the same search like
this::
...
The :class:`semlix.qparser.FuzzyTermPlugin` lets you search for "fuzzy" terms,
...
plugin with the :class:`semlix.qparser.SequencePlugin`, which allows any query
...
``group``
    ...
    This will be the :class:`semlix.qparser.AndGroup` or
    :class:`semlix.qparser.OrGroup` class (*not* an instantiated object) unless
    you've written your own custom grouping syntax you want to use.

``termclass``
    The query class to use to wrap single terms.

    This must be a :class:`semlix.query.Query` subclass (*not* an instantiated
    object) that accepts a fieldname string and term text unicode string in its
    ``__init__`` method. The default is :class:`semlix.query.Term`.

    This is useful if you want to change the default term class to
    :class:`semlix.query.Variations`, or if you've written a custom term class
    you want the parser to use instead of the ones shipped with semlix.

::

    >>> from semlix.qparser import QueryParser, OrGroup
...
The :meth:`semlix.qparser.QueryParser.add_plugin`,
:meth:`semlix.qparser.QueryParser.remove_plugin_class`, and
:meth:`semlix.qparser.QueryParser.replace_plugin` methods let you manipulate
...
* Create a new :class:`semlix.qparser.syntax.GroupNode` subclass to hold
  nodes affected by your operator. This object is responsible for generating
  a :class:`semlix.query.Query` object corresponding to the syntax.
...
For example, if you were creating a ``BEFORE`` operator::
    ...
    from semlix import qparser, query
...
Create an :class:`~semlix.qparser.plugins.OperatorsPlugin` instance with your
