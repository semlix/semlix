...
semlix can quickly suggest replacements for mis-typed words by returning
...
See the :meth:`semlix.spelling.Corrector.suggest` method documentation
...
In semlix 2.7 and later, spelling suggestions are available on all fields.
...
You can then use the :meth:`semlix.searching.Searcher.corrector` method
...
To create a :class:`semlix.spelling.Corrector` object from a sorted word list::

    from semlix.spelling import ListCorrector
...
:class:`semlix.spelling.MultiCorrector`::
...
:meth:`semlix.searching.Searcher.correct_query` method::
...
``query``
    A corrected :class:`semlix.query.Query` tree. You can test
...
You can use a :class:`semlix.highlight.Formatter` object to format the
corrected query string. For example, use the
:class:`~semlix.highlight.HtmlFormatter` to format the corrected string
as HTML::
...
:meth:`semlix.searching.Searcher.correct_query` for information on the
