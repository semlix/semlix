...
* semlix 2.5 will read existing indexes, but segments created by 2.5 will not
  be readable by older versions of semlix.

* As a replacement for field caches to speed up sorting, semlix now supports
  adding a ``sortable=True`` keyword argument to fields. This makes semlix store
  ...
  however it will use more RAM and be slower as semlix caches the field values
  in memory.)
...
* semlix will now detect common types of OR queries and use optimized read-ahead
  matchers to speed them up by several times.

* semlix now includes pure-Python implementations of the Snowball stemmers and
  stop word lists for various languages adapted from NLTK. These are available
  through the :class:`semlix.analysis.LanguageAnalyzer` analyzer or through the
  ``lang=`` keyword argument to the
  :class:`~semlix.fields.TEXT` field.

* You can now use the
  :meth:`semlix.filedb.filestore.Storage.create()` and
  :meth:`semlix.filedb.filestore.Storage.destory()`
...
* By default, semlix now assembles the individual files of a segment into a
  single file when committing. This has a small performance penalty but solves
  a problem where semlix can keep too many files open. semlix is also now
...
* Added a :class:`semlix.query.Regex` term query type, similar to
  :class:`semlix.query.Wildcard`. The parser does not allow regex term queries
  by default. You need to add the :class:`semlix.qparser.RegexPlugin` plugin.
  After you add the plugin, you can use ``r"expression"`` query syntax for
  regular expression term queries. For example, ``r"foo.*bar"``.

* Added the :class:`semlix.qparser.PseudoFieldPlugin` parser plugin. This
...
* The ``groupids`` keyword argument to ``Searcher.search`` has been removed.
  Instead you can now pass a :class:`semlix.sorting.FacetMap` object to the
  ...
  :class:`semlix.sorting.FacetType`` objects to set custom grouping per facet.
...
* Added ``allow_overlap`` option to :class:`semlix.sorting.StoredFieldFacet`.

* In :meth:`~semlix.writing.IndexWriter.add_document`, You can now pass
...
* Added :class:`semlix.analysis.CompoundWordFilter` and
  :class:`semlix.analysis.TeeFilter`.
...
* semlix is now compatible with Python 3 (tested with Python 3.2). Special
...
* Completely revamped spell-checking to make it much faster, easier, and more
  ...
  :meth:`semlix.searching.Searcher.suggest`::

      suglist = searcher.suggest("content", "werd", limit=3)

  semlix now includes convenience methods to spell-check and correct user
  queries, with optional highlighting of corrections using the
  ``semlix.highlight`` module::

      from semlix import highlight, qparser
...
* :class:`semlix.query.FuzzyTerm` now uses the new word graph feature as well
...
* You can now set a boost factor for individual documents as you index them,
  ...
  documentation for the :meth:`~semlix.writing.IndexWriter.add_document` for
  more information.

* Added built-in recording of which terms matched in which documents. Use the
  ``terms=True`` argument to :meth:`semlix.searching.Searcher.search` and use
  :meth:`semlix.searching.Hit.matched_terms` and
  :meth:`semlix.searching.Hit.contains_term` to check matched terms.

* semlix now supports whole-term quality optimizations, so for example if the
...
* Re-architected the query parser to make the code easier to understand. Custom
  parser plugins from previous versions will probably break in semlix 2.0.
...
* The term quality optimizations required changes to the on-disk formats.
  semlix 2.0 if backwards-compatible with the old format. As you rewrite an
  index using semlix 2.0, by default it will use the new formats for new
...
* Reader iteration methods (``__iter__``, ``iter_from``, ``iter_field``, etc.)
  now yield :class:`semlix.reading.TermInfo` objects.

* The arguments to :class:`semlix.query.FuzzyTerm` changed.
