"""
This module contains EXPERIMENTAL support for storing a semlix index's files in
...
To create a new index::
    ...
    from semlix.filedb.gae import DatastoreStorage
...
from semlix.compat import BytesIO
from semlix.index import TOC, FileIndex, _DEF_INDEX_NAME
from semlix.filedb.filestore import ReadOnlyError, Storage
from semlix.filedb.structfile import StructFile
...
class MemcacheLock(object):
    ...
    def acquire(self, blocking=False):
        val = memcache.add(self.name, "L", 360, namespace="semlixlocks")
        ...
        if blocking and not val:
            ...
            while not val:
                time.sleep(0.1)
                val = memcache.add(self.name, "", 360, namespace="semlixlocks")

        return val

    def release(self):
        memcache.delete(self.name, namespace="semlixlocks")


class DatastoreStorage(Storage):
    """An implementation of :class:`semlix.store.Storage` that stores files in
